  ...
  if #terms ~= 0 then
    for i = 1, #terms do
      if terms[i].display_name and terms[i].display_name ~= '' and terms[i].display_name:find('pio', 1) then
        local name_splt = utils.strsplit(terms[i].display_name, ':')
...
local function send(term, cmd)
  vim.fn.chansend(term.job_id, cmd .. enter())
...
local function PioTermClose(t)
  local orig_window = tonumber(utils.strsplit(t.display_name, ':')[2])
  ...
  if string.find(command, ' monitor') then
    ...
    if prev.cli then
      prev.cli.on_close = function(t)
        local ow = tonumber(utils.strsplit(t.display_name, ':')[2])
  ...
  local termConfig = {
    ...
    on_open = function(t)
      ...
      if config.debug then
        local name_splt = utils.strsplit(t.display_name, ':')
        vim.api.nvim_echo({
          { 'ToggleTerm ', 'MoreMsg' },
    ...
    on_close = function(t)
      orig_window = tonumber(utils.strsplit(t.display_name, ':')[2])
    ...
    on_create = function(t)
      local platformio = vim.api.nvim_create_augroup(utils.strsplit(t.display_name, ':')[1], { clear = true })
      ...
      vim.api.nvim_create_autocmd('BufUnload', {
        ...
        callback = function(args)
          ...
          vim.api.nvim_clear_autocmds({ group = utils.strsplit(t.display_name, ':')[1] })
        end,
      })
    end,
  }
